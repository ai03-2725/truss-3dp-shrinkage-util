...
- A webapp gives immediate results based on measurements without needing to rely on manual math.  
- The webapp is focused on simplicity and ease of use.  
...
The webapp stores the known values as well as allowing for export and import of such values for backup and restore.  
...
## Todo
- ~~Write the webapp portion to automate calculations~~ — done: `truss-webui/` builds `<truss-calibrator>`, a zero-config custom element implementing both the quad and single-beam flows, the extrapolation factor storage, and printer-data export/import. See [`truss-webui/README.md`](./truss-webui/README.md) for setup and embedding.
- Embed the built element in the website (the widget is finished; mounting it on the live page is the remaining step).
