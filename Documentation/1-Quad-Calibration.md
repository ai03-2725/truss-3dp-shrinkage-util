...
1. Download the Quad STL file from [here](/Truss%20Calibration%20Beam%20Quad.stl).
    ![Quad design](./Images/truss-quad.png)
1. Load it into a modern slicer of your preference - this guide will cover OrcaSlicer/Bambu Studio.  
1. Slice it with settings which will print accurately and reliably (i.e. no warping, curling, extreme dimensional inaccuracy from overspeed, etc.)  
    ![Slicer loaded](./Images/slicer-loaded.png)
1. In the preview, make sure seams are not placed on the measurement surfaces.  
...
1. Locate the X-beam along the X axis - this is marked with a X label on the print.  
...
1. Repeat the two inner/outer measurements for all 4 axies.
...
1. If calibrating manually, do the following:
    ...
    1. Divide the value by 140. The resulting value (say somewhere in the 0.95~0.99 range) is the shrinkage value of the filament.  
...
1. If your printer's skew is calibratable and tuned to zero, you can stop here and reference the [single-beam page](./2-Single-Calibration.md) in the future.  
    If your printer's skew is not zero (not perfectly calibratable or not adjustable to begin with), do the following:  
    ...
    From now on, you can print just the single X axis beam to save time, then multiply its average measured value by this extrapolation factor to estimate what the average would be if a quad-beam Truss were printed.  
